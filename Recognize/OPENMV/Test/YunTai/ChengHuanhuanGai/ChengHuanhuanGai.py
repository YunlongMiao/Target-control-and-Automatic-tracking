...
servo_pan = pyb.Servo(servo_rotation) # 初始化自转轴舵机1
servo_tilt = pyb.Servo(servo_pitch) # 初始化俯轴舵机2
...
def laser_move_by_degress(rotation, pitch):
    ...
    if servo_rotation_limit[0] < rotation < servo_rotation_limit[1]:
        servo_pan.angle(rotation)
        #servo.degree(1,rotation)
    if servo_pitch_limit[0] < pitch < servo_pitch_limit[1]:
        servo_tilt.angle(pitch)
        #servo.degree(2,pitch)
...
#def screen_function_touch_button():
    #global laser_on_pencil_servo_value,function_running_num
        ...
        ## buzzer.frequency(0)
                ## else:   #在第二页面
    #if flag_key2==2 and pin1.value()==0:
        ...
        ## buzzer.frequency(0)
            ...
            ##         buzzer.frequency(0)
            ##     else: #在第二页面
    #if flag_key2==3 and pin1.value()==0:
        #unction_running_num = 4    #A4靶纸黑框校准
        ## buzzer.frequency(500)
        #sleep(0.5)
        ## buzzer.frequency(0)
        ## else:
        ...
        #while True:
            ...
            ## if screen.touch_exist():    #触摸任意位置返回
            #if flag_key3==2 and pin2.value()==0:
                ...
                #break
            ## else:   #右下角按键，固定为暂停
            #if flag_key3==3 and pin2.value()==0:
                ## buzzer.frequency(500)
                #sleep(0.5)
                ## buzzer.frequency(0)
                #while True:
                    ...
                    ## if screen.touch_exist():
                    ...
                    #if flag_key3==4 and pin2.value()==0:
                        #function_running_num = 3 #绕铅笔路径移动
                                ## buzzer.frequency(500)
                        #sleep(0.5)
                                ## buzzer.frequency(0)
                        #break
                            ## else:   #第二页右上角按键
                    #if flag_key3==5 and pin2.value()==0:
                        #function_running_num = 4 #A4靶纸黑框校准
                                ## buzzer.frequency(500)
                        #sleep(0.5)
                                ## buzzer.frequency(0)
                        #break
                    #if flag_key3==6 and pin2.value()==0:
                        ## else:   #下面两个按键：取消暂停
                            ## buzzer.frequency(500)
                        #sleep(0.5)
                            ## buzzer.frequency(0)
...
while True:
    ...
    print("flag_key1%d",key1_sum)
    print("flag_key2%d",key2_sum)
    print("flag_key3%d",key3_sum)
    time.sleep_ms(500)
